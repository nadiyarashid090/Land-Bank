...
export default function OwnerInput({
  ...
  const handleChange = (e) => {
    ...
    if (searchValue) {
      setOwnerStatus(
        orginalOwnerStatus.filter((owner) =>
          owner.ownerName.toUpperCase().startsWith(searchValue)
        )
      );
    } else {
  ...
  return (
    <div
      className=" border border-primary-input py-3 px-2 rounded-[10px] relative text-left"
      onClick={() => {
        setVisibleInput(visibleInput === "Owner" ? null : "Owner");
      }}
    >
